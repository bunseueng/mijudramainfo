...
import { fetchMovie, fetchPerson, fetchTv } from "@/app/actions/fetchMovieApi";
...
type FetchingPersonSearch = {
  dynamicSearch: [];
  isFetching: boolean;
  searchQuery: string;
  mediaIds: number[];
  setMediaIds: (mediaIds: any) => void;
  openSearch: boolean;
...
const FetchingPersonSearch: React.FC<FetchingPersonSearch> = ({
  dynamicSearch,
  isFetching,
  searchQuery,
  mediaIds,
  ...
  const onClickAddMedia = async (id: string, mediaType: string) => {
    ...
    if (!mediaIds.includes(parsedId)) {
      setMediaIds((prevMediaIds: any) => [...prevMediaIds, parsedId]);
    ...
    try {
      ...
      if (mediaType === "movie") {
        ...
        mediaDetail = await fetchPerson(parsedId.toString());
      }
      if (query === "query") {
...
export default FetchingPersonSearch;
