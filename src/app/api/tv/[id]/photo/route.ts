...
export async function POST(req: Request, { params }: { params: { id: string } }) {
    try {
      ...
      if (cover) {
        const imgId = existingDrama?.public_photo_id;
        if (imgId) {
      ...
      const uploadRes = await cloudinary.uploader.upload(photo, {
        upload_preset: "mijudrama_drama_photo"
      });
  
      if (uploadRes) {
        if (existingDrama) {
          // Fetch existing photos from the movie
          const existingPhotos = existingDrama.photo || [];
          ...
          recordChange('photo', existingDrama.photo, photo);
