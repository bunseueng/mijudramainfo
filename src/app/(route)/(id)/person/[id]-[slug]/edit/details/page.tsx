...
export async function generateMetadata(props: {
  ...
  }-${spaceToHyphen(data.person?.results[0]?.name)}/edit/details`;
  const person_detail = data?.person?.results?.find(
    (p: any) => p.id === Number(person_id) // Convert person_id to number
  );
  return {
    title: `${person_detail?.name} (${person_detail?.original_name}) | Edit Details`,
...
const PersonDetailsPage = async (props: {
  params: Promise<{ "id]-[slug": string }>;
}) => {
  const params = await props.params;
  if (!params["id]-[slug"]) {
