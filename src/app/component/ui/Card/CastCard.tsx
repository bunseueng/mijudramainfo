        ...
        >
          <div className="flex flex-row items-center justify-between">
            {item?.profile_path !== null ? (
              <Link href={`/person/${item?.id}`}>
                <Image
                  ...
                  height={200}
                  quality={100}
                  className="inline-block size-[120px] object-cover rounded-full hover:opacity-75 transform duration-300 cursor-pointer"
                />
              </Link>
            ) : (
              <Link href={`/person/${item?.id}`}>
            ...
            <div className="flex flex-col items-start ml-2">
              <Link
                href={`/person/${item?.id}`}
                className="text-lg md:text-xl font-bold truncate dark:text-[#2196f3] hover:opacity-75 transform duration-300 cursor-pointer"
              >
                {item?.name}
              </Link>
