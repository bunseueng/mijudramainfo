...
import { fetchNewestMovie, fetchRatings } from "@/app/actions/fetchMovieApi";
...
const NewestMovie = ({ getMovie, personDB }: MovieProps) => {
  ...
  const { data: newestMovie, isLoading: isMovieLoading } = useQuery({
  ...
  const items = newestMovie?.total_results;
  const totalItems = newestMovie?.results?.slice(start, end) || []; // Use slice on results
  ...
  if (!newestMovie || !totalItems || !tvRating) {
    return <SearchLoading />;
  }
  return (
