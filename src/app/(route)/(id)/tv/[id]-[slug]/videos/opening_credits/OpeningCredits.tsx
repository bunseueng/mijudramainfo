...
const OpeningCredits: React.FC<TrailerType> = ({ trailer, tv }) => {
  ...
  return (
    <>
      {trailer?.results?.filter((type: any) => type?.type === "Opening Credits")
        ?.length === 0 ? (
        ...
        <div className="flex flex-col relative float-left w-full md:w-[75%] -px-3 py-3 my-10">
          {trailer?.results
            ?.filter((type: any) => type?.type === "Opening Credits")
