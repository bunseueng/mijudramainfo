...
const MovieBehindTheScenes: React.FC<TvTrailerType> = ({ movie_id, movie }) => {
    ...
    <>
      {movieTrailer?.results?.filter(
        ...
        <p className="relative float-left w-full md:w-[75%] text-center md:text-start -px-3 py-3 my-10">
          There no Behind the Scenes for {movie?.title || movie?.name} yet!
        </p>
      ) : (
        <div className="flex flex-col relative float-left w-full md:w-[75%] -px-3 py-3 my-10">
          {movieTrailer?.results
            ?.filter((type: any) => type?.type === "Behind the Scenes")
...
export default MovieBehindTheScenes;
