...
interface tvDetails extends Drama {
  genres_tags?: GenresTagsData[];
}

const GenresAndTags: React.FC<tvId & { tvDetails?: tvDetails | null }> = ({
  tv_id,
  tvDetails,
}) => {
  ...
  useEffect(() => {
    if (tvDetails?.genres_tags && tvDetails.genres_tags.length > 0) {
      const genresTagsData = tvDetails.genres_tags[0];
      if (
        genresTagsData &&
      ...
      const initialGenres = tv.genres.map((genre: any) => ({
        label: genre.name,
        value: genre.name,
      }));
      const initialTags = keywords?.results?.map((k: any) => ({
        name: k?.name,
        value: k?.name,
      }));
      setGenres(initialGenres);
