...
const Production: React.FC<movieId & Movie> = ({ movie_id, movieDetails }) => {
  ...
  const [isItemDataChanged, setIsItemDataChanged] = useState<boolean[]>(
    Array(movieDetails?.production_information?.length).fill(false)
  );
  ...
  const changeHandler = (selectedOptions: any) => {
    setCountries(selectedOptions || []); // If no options are selected, set countries to an empty array
  };
          ...
          production_information: [
            {
              language: language
                ...
                : production_language?.find((p) =>
                    database?.map((i) =>
                      i?.original_language?.includes(p?.original)
                    )
                  )?.value,
              country: countries,
            },
  ...
  useEffect(() => {
    if (
      movieDetails?.production_information &&
      movieDetails?.production_information.length > 0
    ) {
      setDatabase(movieDetails?.production_information);
      setOriginalLanguage(
        movieDetails?.production_information.map((info: any) => info?.language)
      );
      setCountries(
        movieDetails?.production_information.flatMap(
  ...
  useEffect(() => {
    ...
    const combinedData = movieDetails?.production_information || [];
                    ...
                    value={
                      language[idx] // If the user has selected a language, show the selected language
                        ...
                        : production_language?.find(
                          ...
                          )?.value // Otherwise, use the original language
                    }
              ...
              >
                <Select
                  ...
                  options={production_country?.map((country) => ({
                    label: country.value, // Label shown in dropdown
                    value: country.value, // Value used for storage
                  }))}
                  // Fix the value prop to match the expected format
                  value={countries}
