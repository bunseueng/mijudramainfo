...
const MovieCrew: React.FC<movieId & Movie> = ({ movie_id, movieDetails }) => {
  ...
  useEffect(() => {
    if (item.length > 0 && originalItems.length === 0) {
      // Only set originalItems once, when item is initially populated
      setOriginalItems(deepCopyArray(item)); // Make sure it's a deep copy
    }
  ...
  const handleResetAll = async () => {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, 3000)); // Simulating async process

      setCrewRoles([]); // Reset the crew roles
      setHasReordered(false); // Reset reordered flag
      setIsItemChanging([]); // Reset item change flags
      setMarkedForDeletion([]); // Reset deletion flags

      // Restore original order
      if (originalItems.length > 0) {
        setItem(deepCopyArray(originalItems)); // Reset item to original order using deep copy
      }
    } catch (error) {
      console.error(error);
    } finally {
                          ...
                          {openDropdown === `job-${ind}` && (
                            <AnimatePresence>
                              <motion.ul
                                ...
                                exit={{ opacity: 0, y: -10 }}
                                className="w-[200px] md:w-[250px] h-[250px] absolute bg-white dark:bg-[#242424] border-[1px] border-[#dcdfe6] dark:border-[#242424] py-1 mt-2 rounded-md z-50 custom-scroll"
                              >
