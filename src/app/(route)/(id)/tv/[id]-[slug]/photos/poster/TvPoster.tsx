...
const TvPoster = ({ getDrama, tv_id }: DramaDatabase) => {
  ...
  const currentBackdrops = tv?.images?.posters?.map((item: any) => item);
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await fetch(`/api/tv/${tv_id}/photo`, {
        method: "POST",
        headers: {
        ...
        <div className="max-w-6xl mx-auto flex items-center mt-0 px-4 py-2">
          <div className="flex items-center lg:items-start px-2 cursor-default">
            ...
            <div className="flex flex-col pl-5 py-2">
              ...
              <Link
                prefetch={false}
                href={`/tv/${tv_id}-${spaceToHyphen(tv?.name || tv?.title)}`}
