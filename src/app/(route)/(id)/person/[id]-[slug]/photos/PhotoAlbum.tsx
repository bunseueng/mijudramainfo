...
const PersonPhotoAlbum = ({ getPerson, personId }: DramaDatabase) => {
  ...
  const currentProfile = getImage?.profiles?.map((item: any) => item);
  const currentPosters = getPerson?.map((item: any) => item?.photo);
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      const response = await fetch(`/api/person/${personId}/photo`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export default PersonPhotoAlbum;
